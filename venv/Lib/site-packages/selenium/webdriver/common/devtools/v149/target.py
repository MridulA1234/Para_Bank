...
class TargetInfo:
    ...
    def from_json(cls, json):
        return cls(
            ...
            parent_id=TargetID.from_json(json['parentId']) if 'parentId' in json else None,
...
def create_target(
    ...
    :param hidden: **(EXPERIMENTAL)** *(Optional)* Whether to create a hidden target. The hidden target is observable via protocol, but not present in the tab 01_UI strip. Cannot be created with ```forTab: true````, ````newWindow: true```` or ````background: false```. The life-time of the tab is limited to the life-time of the session.
